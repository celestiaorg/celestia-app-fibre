...
// deployObservabilityPayloadDirect copies an observability archive to the observability monitoring host.
...
func checkForRunningExperiments(ctx context.Context, cfg Config) error {
	...
	if cfg.DigitalOceanToken != "" {
		...
		if err != nil {
			log.Printf("Warning: failed to check DigitalOcean for running experiments: %v", err)
		} else if running {
			hasRunningExperiments = true
			log.Printf("Found experiment '%s' with chainID '%s' already running in DigitalOcean", cfg.Experiment, cfg.ChainID)
		}
	}
	...
	if cfg.GoogleCloudProject != "" {
		...
		if err != nil {
			log.Printf("Warning: failed to create Google Cloud client options: %v", err)
		} else {
			running, err := checkForRunningGCExperiments(ctx, cfg.GoogleCloudProject, opts, cfg.Experiment, cfg.ChainID)
			if err != nil {
				log.Printf("Warning: failed to check Google Cloud for running experiments: %v", err)
			} else if running {
				hasRunningExperiments = true
				log.Printf("Found experiment '%s' with chainID '%s' already running in Google Cloud", cfg.Experiment, cfg.ChainID)
			}
		}
	}
