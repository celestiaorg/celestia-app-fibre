...
func (s Set) NewSignatureSet(targetVotingPower, targetValidatorsCount cmtmath.Fraction, requiredBytesSigned []byte) *SignatureSet {
	...
	requiredCount := len(s.Validators) * int(targetValidatorsCount.Numerator) / int(targetValidatorsCount.Denominator)
	...
	return &SignatureSet{
		...
		signatures:          make([][]byte, 0, len(s.Validators)),
...
func (ss *SignatureSet) Miss(_ *core.Validator) {
	ss.mu.Lock()
	defer ss.mu.Unlock()
