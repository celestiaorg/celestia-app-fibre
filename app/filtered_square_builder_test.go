...
func TestAddPayForFibreTxWithSystemBlob(t *testing.T) {
	encConf := encoding.MakeConfig(ModuleEncodingRegisters...)
	txConfig := encConf.TxConfig

	tests := []struct {
		name              string
		createMsg         func() *fibretypes.MsgPayForFibre
		...
		{
			name: "success - valid PayForFibre tx with system blob",
			createMsg: func() *fibretypes.MsgPayForFibre {
				privKey := secp256k1.GenPrivKey()
				pubKey := privKey.PubKey()
				secp256k1PubKey := *pubKey.(*secp256k1.PubKey)
		...
		{
			name: "error - system blob creation fails (strict mode)",
			createMsg: func() *fibretypes.MsgPayForFibre {
				privKey := secp256k1.GenPrivKey()
				pubKey := privKey.PubKey()
				secp256k1PubKey := *pubKey.(*secp256k1.PubKey)
				...
				// Invalid namespace (wrong size)
				invalidNamespace := []byte{1, 2, 3}
			...
			wantError:         true,
			wantErrorContains: "failed to create system blob",
			wantReverted:      false, // In strict mode, we return error before reverting
		},
		{
			name: "error - system blob creation fails (non-strict mode)",
			createMsg: func() *fibretypes.MsgPayForFibre {
				privKey := secp256k1.GenPrivKey()
				pubKey := privKey.PubKey()
				secp256k1PubKey := *pubKey.(*secp256k1.PubKey)
			...
			wantError:    true,
			wantReverted: true, // In non-strict mode, we revert on error
		},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			// Create PayForFibre transaction
			msg := tt.createMsg()
			...
			// Append the transaction first (as required by the function)
			appended := builder.AppendPayForFibreTx(txBytes)
			require.True(t, appended, "transaction should be appended")

			// Count PayForFibre transactions before
			payForFibreCountBefore := len(builder.PayForFibreTxs)

			// Call the function
			err = addPayForFibreTxWithSystemBlob(builder, txBytes, msg, tt.opts)
			...
			if tt.wantError {
				...
				if tt.wantReverted {
					// Verify transaction was reverted
					payForFibreCountAfter := len(builder.PayForFibreTxs)
					require.Equal(t, payForFibreCountBefore-1, payForFibreCountAfter, "transaction should be reverted")
				}
			} else {
				require.NoError(t, err)
				// Verify transaction was not reverted
				payForFibreCountAfter := len(builder.PayForFibreTxs)
				require.Equal(t, payForFibreCountBefore, payForFibreCountAfter, "transaction should not be reverted")
			}
		})
	}
}

func TestAddPayForFibreTxsToBuilder(t *testing.T) {
	encConf := encoding.MakeConfig(ModuleEncodingRegisters...)
	txConfig := encConf.TxConfig
	...
	tests := []struct {
		...
		wantError         bool
		wantErrorContains string
		wantProcessed     int // How many transactions should be successfully processed
	}{
		{
			name: "success - multiple valid PayForFibre txs",
			createTxs: func() [][]byte {
				txs := make([][]byte, 3)
				for i := 0; i < 3; i++ {
					txs[i] = createPayForFibreTx(t, txConfig)
				}
				return txs
			},
			opts: PayForFibreOptions{
		...
		{
			name: "error - decode failure (strict mode)",
			createTxs: func() [][]byte {
				// Invalid transaction bytes
				return [][]byte{[]byte("invalid tx bytes")}
			},
			opts: PayForFibreOptions{
				StrictErrorHandling: true,
			},
			wantError:         true,
			wantErrorContains: "failed to decode pay-for-fibre tx",
			wantProcessed:     0,
		},
		{
			name: "error - decode failure (non-strict mode)",
			createTxs: func() [][]byte {
				// Invalid transaction bytes
				return [][]byte{[]byte("invalid tx bytes")}
			},
			...
			wantError:     false, // Non-strict mode continues on error
			wantProcessed: 0,
		},
		{
			name: "mixed - some succeed, some fail (non-strict mode)",
			createTxs: func() [][]byte {
				validTx1 := createPayForFibreTx(t, txConfig)
				validTx2 := createPayForFibreTx(t, txConfig)
				invalidTx := []byte("invalid tx bytes")
				return [][]byte{validTx1, invalidTx, validTx2}
			},
			opts: PayForFibreOptions{
				StrictErrorHandling: false,
				OnError: func(txHash []byte, err error, reason string) {
					// Verify error callback is called for invalid tx
				},
			},
			wantError:     false,
			wantProcessed: 2, // Two valid transactions should be processed
		},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			txs := tt.createTxs()
			err = addPayForFibreTxsToBuilder(builder, txs, txConfig, tt.opts)
			...
			// Verify number of processed transactions
			require.Equal(t, tt.wantProcessed, len(builder.PayForFibreTxs), "number of processed PayForFibre transactions")
		})
	}
}

func TestBuildSquareFromSeparatedTxs(t *testing.T) {
	encConf := encoding.MakeConfig(ModuleEncodingRegisters...)
	txConfig := encConf.TxConfig
	...
	tests := []struct {
		...
		wantErrorContains  string
		wantSquareSize     int
		wantPayForFibreTxs int
	}{
		{
			name:           "success - normal txs + blob txs + PayForFibre txs",
			normalTxs:      [][]byte{createNormalTx(t, txConfig)},
			blobTxs:        []*tx.BlobTx{createBlobTxAsBlobTx(t)},
			payForFibreTxs: [][]byte{createPayForFibreTx(t, txConfig)},
			opts: PayForFibreOptions{
				StrictErrorHandling: true,
			},
			wantError:          false,
			wantSquareSize:     1, // At least 1x1 square
			wantPayForFibreTxs: 1,
		},
		{
			name:           "success - only PayForFibre txs",
			normalTxs:      [][]byte{},
			blobTxs:        []*tx.BlobTx{},
			payForFibreTxs: [][]byte{createPayForFibreTx(t, txConfig), createPayForFibreTx(t, txConfig)},
			opts: PayForFibreOptions{
				StrictErrorHandling: true,
			},
			wantError:          false,
			wantSquareSize:     1,
			wantPayForFibreTxs: 2,
		},
		{
			name:           "success - empty transactions",
			normalTxs:      [][]byte{},
			blobTxs:        []*tx.BlobTx{},
			payForFibreTxs: [][]byte{},
			opts: PayForFibreOptions{
			...
			wantError:          false,
			wantSquareSize:     1, // Empty square is still 1x1
			wantPayForFibreTxs: 0,
		},
		{
			name:           "error - PayForFibre tx fails (strict mode)",
			normalTxs:      [][]byte{},
			blobTxs:        []*tx.BlobTx{},
			payForFibreTxs: [][]byte{[]byte("invalid tx bytes")},
			opts: PayForFibreOptions{
				StrictErrorHandling: true,
			},
			wantError:         true,
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			square, err := buildSquareFromSeparatedTxs(
				tt.normalTxs,
				tt.blobTxs,
				tt.payForFibreTxs,
				txConfig,
			...
			if tt.wantError {
				...
				require.NoError(t, err)
				require.NotNil(t, square)
				require.GreaterOrEqual(t, square.Size(), tt.wantSquareSize)

				// Verify PayForFibre transactions are in the square
				if tt.wantPayForFibreTxs > 0 {
					payForFibreShareRange := share.GetShareRangeForNamespace(square, share.PayForFibreNamespace)
					require.False(t, payForFibreShareRange.IsEmpty(), "PayForFibre namespace should have shares")
				}
			}
...
func TestBuildSquareFromSeparatedTxsSystemBlobs(t *testing.T) {
	encConf := encoding.MakeConfig(ModuleEncodingRegisters...)
	txConfig := encConf.TxConfig

	// Create PayForFibre transactions
	payForFibreTx1 := createPayForFibreTx(t, txConfig)
	payForFibreTx2 := createPayForFibreTx(t, txConfig)

	// Decode to get the namespace from PaymentPromise
	sdkTx1, err := txConfig.TxDecoder()(payForFibreTx1)
	require.NoError(t, err)
	msgPayForFibre1, hasPayForFibre1 := extractMsgPayForFibre(sdkTx1)
	require.True(t, hasPayForFibre1)

	expectedNs1, err := share.NewNamespaceFromBytes(msgPayForFibre1.PaymentPromise.Namespace)
	require.NoError(t, err)
	...
	square, err := buildSquareFromSeparatedTxs(
		[][]byte{},
		[]*tx.BlobTx{},
		[][]byte{payForFibreTx1, payForFibreTx2},
		txConfig,
		64,
		64,
		PayForFibreOptions{
	...
	// Verify system blobs are included in the square
	nsRange := share.GetShareRangeForNamespace(square, expectedNs1)
	require.False(t, nsRange.IsEmpty(), "System blob should be in the square")

	// Verify system blobs have correct namespace and share version 2
	// Range.End is exclusive, so we use nsRange.End directly
	require.LessOrEqual(t, nsRange.End, len(square), "Range end should be within square bounds")
	blobShares := square[nsRange.Start:nsRange.End]
	require.Greater(t, len(blobShares), 0)
	firstShare := blobShares[0]
	require.Equal(t, share.ShareVersionTwo, firstShare.Version())
	require.True(t, firstShare.Namespace().Equals(expectedNs1))
}
